...
**Total: 30 active entries across 9 categories · 10 structurally prevented**
...
> repository passed and the plan's verification table recorded that row as
> PASS. The new `testing.md` entry is about the check, not the file: it only
> ever ran in one direction, and a one-directional diff cannot see an absence.
> It is **not** marked structurally prevented — the reverse check has been run
> by hand once, but nothing yet runs it automatically.
